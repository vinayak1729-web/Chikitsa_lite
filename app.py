...
# Gemini 2.5 Pro streaming function
def gemini_chat(user_input, system_prompt=SYSTEM_PROMPT):
    """Stream responses from Gemini 2.5 Pro model."""
    try:
        ...
        logging.error(f"Gemini 2.5 Pro error: {str(e)}")
        # Fallback to Gemma3n
        for chunk in gemma_chat_stream(user_input, system_prompt):
            yield chunk

# Gemma3n streaming function
def gemma_chat_stream(user_input, system_prompt=SYSTEM_PROMPT):
    """Stream responses from Gemma3n model via Ollama API."""
    try:
        payload = {
            "model": "gemma3n:e2b",
            "prompt": user_input,
        ...
        logging.error(f"Gemma3n error: {str(e)}")
...
def thank_you():
    ...
    if request.headers.get("Accept") == "text/event-stream":
        def generate():
            try:
                report = ""
                for chunk in gemini_chat(questionnaire_prompt + " " + close_ended_str + " " + open_ended_str):
                    report += chunk
    ...
    data = load_wellness_data(username)
    return render_template('thank_you.html', judge_gemini=data.get("wellness_report", "Report not available."), user_name=username, completejudege=data.get("wellness_report"))
...
def feedback():
    ...
    questionnaire_prompt = """
    This is my assessment of close-ended questions and open-ended questions. Please provide feedback on me in friendly tone in summary like a professional psychiatrist.
    """
        ...
        def generate():
            try:
                ...
                if report:
                    ...
                    user_data['wellness_report'] = report
                    write_json_file(user_file, user_data)
            except Exception as e:
                yield f"data: {{'text': 'Error: {str(e)}'}}\n\n"
            yield "data: [DONE]\n\n"
...
def chat():
    ...
    if request.method == 'GET':
        ...
        if request.headers.get("Accept") == "text/event-stream":
            ...
            def generate():
                try:
                    for chunk in gemini_chat(user_input):
                        yield f"data: {json.dumps({'text': chunk})}\n\n"
                except Exception as e:
                    yield f"data: {{'text': 'Error: {str(e)}'}}\n\n"
                yield "data: [DONE]\n\n"
...
def send_message():
    ...
    def generate():
        try:
            if is_online():
                # Use Gemini 2.5 Pro with image support
                client = genai.GenerativeAI(api_key=os.environ.get("GEMINI_API_KEY"))
                model = "gemini-2.5-flash-lite"
                contents = [
                    types.Content(
                        role="user",
                        parts=[
                            types.Part.from_text(text=f"{SYSTEM_PROMPT}\n\n{user_message}"),
                        ] + [types.Part.from_data(data=image, mime_type="image/jpeg") for image in images]
                    ),
                ]
                generate_content_config = types.GenerateContentConfig(
...
def wellness_report():
    ...
    if request.headers.get("Accept") == "text/event-stream":
        def generate():
            try:
                report = ""
                for chunk in gemini_chat(prompt):
                    report += chunk
                    yield f"data: {json.dumps({'text': chunk})}\n\n"
                if report:
                    save_wellness_data(username, wellness_report=report)
...
@login_required
def image_analysis():
    """Handle image analysis using Gemini 2.5 Pro or Gemma3n."""
    if request.method == 'POST':
        if 'file' not in request.files:
            flash('No file uploaded', 'error')
            return redirect(request.url)
        ...
        if uploaded_file and allowed_file(uploaded_file.filename):
            try:
                ...
                image_prompt = """
                <instructions>
                You are CHIKITSA, an AI combining the expertise of a compassionate digital psychiatrist and a skilled medical practitioner specializing in image analysis. Your role is to provide mental health support and analyze medical images (e.g., scans, reports) for a renowned hospital, ensuring empathetic, evidence-based, and inclusive responses. For mental health concerns (from user input or inferred from images), offer coping strategies and integrate practical, knowledge-based lessons from the Bhagavad Gita, focusing solely on psychological/philosophical insights and avoiding religious or spiritual references unless explicitly requested. For medical images, identify health issues, suggest next steps, and recommend consulting a doctor without prescribing remedies. Always recommend meditation and low-intensity exercise to support mental and physical well-being, tailoring suggestions to the user's context. Respond in English, or optionally Hinglish or Minglish (Marathi-English mix) for cultural engagement, maintaining a friendly, professional tone. Structure your responses clearly, ensuring sensitivity to the user's emotional and physical state.
                ...
                if request.headers.get("Accept") == "text/event-stream":
                    def generate():
                        try:
                            for chunk in gemini_chat(f"Please analyze this medical image: {image_prompt}", image_prompt):
                                yield f"data: {json.dumps({'text': chunk})}\n\n"
                        except Exception as e:
                            yield f"data: {{'text': 'Error: {str(e)}'}}\n\n"
                        yield "data: [DONE]\n\n"
                ...
                flash(f'Error analyzing image: {str(e)}', 'error')
                return render_template('image_analysis.html', analysis=None)
        else:
            flash('Invalid file format. Please upload PNG, JPG, JPEG, or GIF files.', 'error')
...
# Emotion detection and attention monitoring (dummy implementation)
@app.route('/talk_to_me', methods=['GET', 'POST'])
@login_required
def talk_to_me():
    """Handles user input with dummy emotion detection message."""
    if request.method == 'GET':
        return render_template('talk_to_me.html', emotion_message="Emotion detection is currently not working.")

    elif request.method == 'POST':
        user_input = request.form.get('user_input', '')
        
        # Dummy emotion and attention status
        emotion_message = "Emotion detection is currently not working."
        prompt = f"{user_input}\n\nNote: Emotion detection is currently unavailable."
        ...
        return jsonify({'response': response, 'emotion_message': emotion_message})

@app.route('/video_feed')
def video_feed():
    """Dummy video feed response indicating emotion detection is not working."""
    return Response("data: {'text': 'Video-based emotion detection is currently not working.'}\n\ndata: [DONE]\n\n", content_type='text/event-stream')
